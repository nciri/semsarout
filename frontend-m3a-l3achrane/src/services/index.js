...
import { listings, currentProfile, threads } from '../data/index.js'
...
// sinon RIEN n'est mocké. Le portail partenaire l'était encore par défaut alors que
// services/partner sert toutes ses routes : l'écran affichait des données inventées en prod.
const ALL_MOCK = import.meta.env.VITE_USE_MOCK === 'true'
const MOCK_DOMAINS = new Set(
  (import.meta.env.VITE_MOCK_DOMAINS ?? '').split(',').filter(Boolean),
)
...
// réservations, subventions, factures, clés API, webhooks, reporting.
// Servi par services/partner : plus aucun repli sur des données inventées.
// ---------------------------------------------------------------------------------------
