...
const FILTERS = [
  ...
  { value: 'waiting', label: 'Mises en attente' },
]
  ...
  if (app.statut === 'pending') {
    return (
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        ...
        <Button size="sm" variant="secondary" onClick={onWait}>Mettre en attente</Button>
        <Button size="sm" variant="ghost" onClick={onRefuse}>Refuser</Button>
      </div>
    )
  }
  if (app.statut === 'waiting') {
    return (
      <Badge tone="warning">En attente — vous répondrez après les autres visites</Badge>
    )
  }
  if (app.statut === 'refused') {
    return (
      <Badge tone="neutral">Refusée — le candidat a été notifié</Badge>
    )
  }
  return null
}
        ...
        {app.statut === 'accepted' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, padding: '14px 16px', borderRadius: 'var(--radius-md)', background: 'var(--navy-50)', border: '1px solid var(--navy-100)' }}>
            <div style={{ font: 'var(--fw-bold) var(--fs-sm) var(--font-display)', color: 'var(--navy-700)' }}>
              Candidature acceptée — proposez un créneau de visite
            </div>
