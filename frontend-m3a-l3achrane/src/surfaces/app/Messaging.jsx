        ...
        <div style={{ overflowY: 'auto', flex: 1 }}>
          {threads.map((t) => (
            <button
              ...
              style={{ display: 'flex', gap: 12, alignItems: 'center', width: '100%', padding: '12px 20px', background: t.id === active?.id ? 'var(--navy-50)' : 'transparent', border: 'none', borderBottom: '1px solid var(--gray-100)', cursor: 'pointer', textAlign: 'left' }}
            >
              <Avatar src={t.avatar} name={t.nom} size={42} />
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ font: 'var(--fw-semibold) var(--fs-body) var(--font-display)', color: 'var(--text-strong)' }}>{t.nom}</span>
                  <span style={{ font: 'var(--fw-regular) var(--fs-xs) var(--font-body)', color: 'var(--text-muted)' }}>{t.heure}</span>
                </div>
                <div style={{ font: 'var(--fw-regular) var(--fs-sm) var(--font-body)', color: 'var(--text-muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.dernier}</div>
              </div>
            </button>
          ))}
        </div>
          ...
          <>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 24px', borderBottom: '1px solid var(--border-subtle)' }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <IconButton icon="phone" label="Appeler" variant="ghost" round />
                <IconButton icon="video" label="Visio" variant="ghost" round />
              </div>
            </div>
              ...
              {active.messages.map((m, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: m.mine ? 'flex-end' : 'flex-start' }}>
                  <div style={{ maxWidth: '68%', padding: '10px 14px', borderRadius: m.mine ? '16px 16px 4px 16px' : '16px 16px 16px 4px', background: m.mine ? 'var(--navy-700)' : '#fff', color: m.mine ? '#fff' : 'var(--text-body)', border: m.mine ? 'none' : '1px solid var(--border-subtle)', font: 'var(--fw-regular) var(--fs-body)/1.45 var(--font-body)', boxShadow: 'var(--shadow-xs)' }}>
                    {m.texte}
                    <div style={{ font: 'var(--fw-regular) 10px var(--font-body)', color: m.mine ? 'rgba(255,255,255,.6)' : 'var(--text-muted)', textAlign: 'right', marginTop: 4 }}>{m.heure}</div>
                  </div>
                </div>
              ))}
              <div ref={endRef} />
