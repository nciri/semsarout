 ...
 * `errorLabel` : libellé du marqueur porté par un niveau dont la dernière
 * synchronisation a échoué (`sync_error`) — sans lui, l'échec d'un niveau resté
 * en arrière-plan n'est visible nulle part tant qu'on ne l'ouvre pas.
 */
export default function LevelTabs({ levels, currentId, onSelect, onCreate, readOnly = false, errorLabel = null }) {
    ...
    <div className="flex items-center gap-2 overflow-x-auto" role="tablist" aria-label={t('dashboard:designEditor.levels.label')}>
      {levels.map((lv) => (
        <button
          ...
          className={`min-h-[44px] px-4 rounded-md border whitespace-nowrap inline-flex items-center gap-2 ${
            lv.id === currentId ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-gray-700 border-gray-300'
          }`}
        >
          {lv.name}
          {errorLabel && lv.sync_error && (
            <FiAlertTriangle
              aria-label={errorLabel}
              role="img"
              className={`w-4 h-4 ${lv.id === currentId ? 'text-white' : 'text-red-600'}`}
            />
          )}
        </button>
      ))}
