...
env UPSTREAM_URL="$MONO" JWT_SECRET_KEY="$JWT" INTERNAL_TOKEN="$ITOK" \
  ...
  PAYMENT_URL=http://localhost:8507 BILLING_URL=http://localhost:8508 \
...
for r in listing catalog identity contract payment billing; do relay "$r"; done
for w in search crm marketplace geo agency messaging analytics billing notification identity audit; do worker "$w"; done
( cd backend; set -a; source .env; set +a
...
for e in "monolithe:7000:/api/v1/properties?per_page=1" "BFF:$BFF_PORT:/health" \
  ...
  contract:8505 legal:8506 payment:8507 billing:8508; do
