...
env UPSTREAM_URL="$MONO" JWT_SECRET_KEY="$JWT" INTERNAL_TOKEN="$ITOK" TENANT_DEV_HEADER=true \
  ...
  RENTAL_URL=http://localhost:8518 COMMISSION_URL=http://localhost:8519 SELLING_URL=http://localhost:8520 \
  COLOC_LISTING_URL=http://localhost:8521 COLOC_PROFILE_URL=http://localhost:8522 \
  MATCHING_URL=http://localhost:8523 \
...
for r in listing catalog identity contract payment billing transactions programs agency crm directory rental commission selling coloc-listing coloc-profile; do relay "$r"; done
for w in search crm marketplace geo agency messaging analytics billing notification identity audit transactions legal contract rental commission coloc-profile matching; do worker "$w"; done
...
for e in "monolithe:7000:/api/v1/properties?per_page=1" "BFF:$BFF_PORT:/health" \
  ...
  rental:8518 commission:8519 selling:8520 coloc-listing:8521 coloc-profile:8522 matching:8523; do
