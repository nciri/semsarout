"""Émission d'`audit.logged` depuis coloc-listing vers le service audit (journal back-office).
...
l'une des deux lignes. Chaque émetteur a donc sa propre plage ; celle de coloc-listing démarre
à `_ID_BASE` (identity : 9_000_000_000_001+, trust-safety : 9_200_000_000_001+).

`entity_id` du journal est un entier, alors que les objets de coloc-listing ont des ids
textuels (uuid) : on le laisse à None et l'id réel va dans `extra_data`.
"""
...
TENANT = "m3a-l3achrane"  # coloc-listing ne sert que ce produit

_SEQ = "coloc_listing.audit_log_seq"
_ID_BASE = 9_100_000_000_001  # début de la séquence — cf. db.init_db()
